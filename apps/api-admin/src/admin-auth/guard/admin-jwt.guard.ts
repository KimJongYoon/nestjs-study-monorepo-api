...
import { AdminAuthJwtGuardException } from '../exception/admin-auth.jwt-guard.exception';
  ...
  private async validateToken(accessToken: string, req: any) {
    try {
      ...
      const payload = await firstValueFrom(
        this.client.send(AuthChannelEnum.ADMIN_VALIDATE_TOKEN, record),
      );

      return payload;
    } catch (error) {
      AdminAuthJwtGuardException.validate(error);
    }
  }
